...
import measure.adaptor

import measure.state_collection
import measure.state
import measure.group
...
def import_users_directory(adaptor,state_collection):
    return _import_users_directory(adaptor.data_location,{})

def import_projects_directory(adaptor,state_collection):
    return _import_projects_directory(adaptor.data_location,{})

def import_directory(adaptor,state_collection):
...
def _import_collection_directory(directory_path, meta):
    return measure.state_collection.StateCollection(states=_import_directory(directory_path,meta),groups_dict={})
...
def _import_projects_directory(directory_path, meta):
    ...
    for project_directory in project_directories:
        ...
        project_group = measure.group.create_group('project_directory',project_states,{}, project_meta)
        groups.append(project_group)
        state_list += project_states

    return measure.state_collection.StateCollection(states=state_list,groups_dict={'project_directory':groups}) 
...
def _import_users_directory(directory_path, meta):
    ...
    for user_directory in user_directories:
        ...
        state_collection= measure.state_collection.join_state_collections( 
            state_collection, 
...
def create_state_from_json(file_location, state_id, meta ):
    with open(file_location) as f:
        ...
        new_state = measure.state.State(id=state_id, data={'json':data},measures={},meta=meta,groups_dict={})
    return new_state
